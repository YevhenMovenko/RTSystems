...
void DS3231_EnableBatterySquareWave(DS3231_State enable){
	...
	DS3231_SetRegByte(DS3231_REG_CONTROL, (control & 0xbf) | ((enable & 0x01) << DS3231_BIT_BBSQW));
}

void DS3231_SetInterruptMode(DS3231_InterruptMode mode){
	uint8_t control = DS3231_GetRegByte(DS3231_REG_CONTROL);
	DS3231_SetRegByte(DS3231_REG_CONTROL, (control & 0xfb) | ((mode & 0x01) << DS3231_BIT_INTCN));
}
...
void DS3231_SetRateSelect(DS3231_Rate rate){
	uint8_t control = DS3231_GetRegByte(DS3231_REG_CONTROL);
	DS3231_SetRegByte(DS3231_REG_CONTROL, (control & 0xe7) | ((rate & 0x03) << DS3231_BIT_RS1));
}
...
void DS3231_EnableOscillator(DS3231_State enable){
	uint8_t control = DS3231_GetRegByte(DS3231_REG_CONTROL);
	DS3231_SetRegByte(DS3231_REG_CONTROL, (control & 0x7f) | ((!enable & 0x01) << DS3231_BIT_EOSC));
}
...
void DS3231_EnableAlarm2(DS3231_State enable){
	uint8_t control = DS3231_GetRegByte(DS3231_REG_CONTROL);
	DS3231_SetRegByte(DS3231_REG_CONTROL, (control & 0xfd) | ((enable & 0x01) << DS3231_BIT_A2IE));
	DS3231_SetInterruptMode(DS3231_ALARM_INTERRUPT);
}
...
void DS3231_ClearAlarm2Flag(){
	uint8_t status = DS3231_GetRegByte(DS3231_REG_CNTR_STATUS) & 0xfd;
	DS3231_SetRegByte(DS3231_REG_CNTR_STATUS, status & ~(0x01 << DS3231_BIT_A2F));
}
...
void DS3231_SetAlarm2Minute(uint8_t minute){
	uint8_t temp = DS3231_GetRegByte(DS3231_REG_ALARM2_MIN) & 0x80;
	uint8_t a2m2 = temp | (DS3231_EncodeBCD(minute) & 0x3f);
	DS3231_SetRegByte(DS3231_REG_ALARM2_MIN, a2m2);
}
...
void DS3231_SetAlarm2Hour(uint8_t hour_24mode){
	uint8_t temp = DS3231_GetRegByte(DS3231_REG_ALARM2_HOUR) & 0x80;
	uint8_t a2m3 = temp | (DS3231_EncodeBCD(hour_24mode) & 0x3f);
	DS3231_SetRegByte(DS3231_REG_ALARM2_HOUR, a2m3);
}
...
void DS3231_SetAlarm2Date(uint8_t date){
	uint8_t temp = DS3231_GetRegByte(DS3231_REG_ALARM2_DAY) & 0x80;
	uint8_t a2m4 = temp | (DS3231_EncodeBCD(date) & 0x3f);
	DS3231_SetRegByte(DS3231_REG_ALARM2_DAY, a2m4);
}
...
void DS3231_SetAlarm2Day(uint8_t day){
	uint8_t temp = DS3231_GetRegByte(DS3231_REG_ALARM2_DAY) & 0x80;
	uint8_t a2m4 = temp | (0x01 << DS3231_BIT_DYDT) | (DS3231_EncodeBCD(day) & 0x3f);
	DS3231_SetRegByte(DS3231_REG_ALARM2_DAY, a2m4);
}
...
void DS3231_SetAlarm2Mode(DS3231_Alarm2Mode alarmMode){
	uint8_t temp;
	temp = DS3231_GetRegByte(DS3231_REG_ALARM1_MIN) & 0x7f;
	DS3231_SetRegByte(DS3231_REG_ALARM2_MIN, temp | (((alarmMode >> 0) & 0x01) << DS3231_BIT_AXMY));
	temp = DS3231_GetRegByte(DS3231_REG_ALARM1_HOUR) & 0x7f;
	DS3231_SetRegByte(DS3231_REG_ALARM2_HOUR, temp | (((alarmMode >> 1) & 0x01) << DS3231_BIT_AXMY));
	temp = DS3231_GetRegByte(DS3231_REG_ALARM1_DAY) & 0x7f;
	DS3231_SetRegByte(DS3231_REG_ALARM2_DAY, temp | (((alarmMode >> 2) & 0x01) << DS3231_BIT_AXMY) | (alarmMode & 0x80));
}
...
void DS3231_EnableAlarm1(DS3231_State enable){
	uint8_t control = DS3231_GetRegByte(DS3231_REG_CONTROL);
	DS3231_SetRegByte(DS3231_REG_CONTROL, (control & 0xfe) | ((enable & 0x01) << DS3231_BIT_A1IE));
	DS3231_SetInterruptMode(DS3231_ALARM_INTERRUPT);
}
...
void DS3231_ClearAlarm1Flag(){
	uint8_t status = DS3231_GetRegByte(DS3231_REG_CNTR_STATUS) & 0xfe;
	DS3231_SetRegByte(DS3231_REG_CNTR_STATUS, status & ~(0x01 << DS3231_BIT_A1F));
}
...
void DS3231_SetAlarm1Second(uint8_t second){
	uint8_t temp = DS3231_GetRegByte(DS3231_REG_ALARM1_SEC) & 0x80;
	uint8_t a1m1 = temp | (DS3231_EncodeBCD(second) & 0x3f);
	DS3231_SetRegByte(DS3231_REG_ALARM1_SEC, a1m1);
}
...
void DS3231_SetAlarm1Minute(uint8_t minute){
	uint8_t temp = DS3231_GetRegByte(DS3231_REG_ALARM1_MIN) & 0x80;
	uint8_t a1m2 = temp | (DS3231_EncodeBCD(minute) & 0x3f);
	DS3231_SetRegByte(DS3231_REG_ALARM1_MIN, a1m2);
}
...
void DS3231_SetAlarm1Hour(uint8_t hour_24mode){
	uint8_t temp = DS3231_GetRegByte(DS3231_REG_ALARM1_HOUR) & 0x80;
	uint8_t a1m3 = temp | (DS3231_EncodeBCD(hour_24mode) & 0x3f);
	DS3231_SetRegByte(DS3231_REG_ALARM1_HOUR, a1m3);
}
...
void DS3231_SetAlarm1Date(uint8_t date){
	uint8_t temp = DS3231_GetRegByte(DS3231_REG_ALARM1_DAY) & 0x80;
	uint8_t a1m4 = temp | (DS3231_EncodeBCD(date) & 0x3f);
	DS3231_SetRegByte(DS3231_REG_ALARM1_DAY, a1m4);
}
...
void DS3231_SetAlarm1Day(uint8_t day){
	uint8_t temp = DS3231_GetRegByte(DS3231_REG_ALARM1_DAY) & 0x80;
	uint8_t a1m4 = temp | (0x01 << DS3231_BIT_DYDT) | (DS3231_EncodeBCD(day) & 0x3f);
	DS3231_SetRegByte(DS3231_REG_ALARM1_DAY, a1m4);
}
...
void DS3231_SetAlarm1Mode(DS3231_Alarm1Mode alarmMode){
	uint8_t temp;
	temp = DS3231_GetRegByte(DS3231_REG_ALARM1_SEC) & 0x7f;
	DS3231_SetRegByte(DS3231_REG_ALARM1_SEC, temp | (((alarmMode >> 0) & 0x01) << DS3231_BIT_AXMY));
	temp = DS3231_GetRegByte(DS3231_REG_ALARM1_MIN) & 0x7f;
	DS3231_SetRegByte(DS3231_REG_ALARM1_MIN, temp | (((alarmMode >> 1) & 0x01) << DS3231_BIT_AXMY));
	temp = DS3231_GetRegByte(DS3231_REG_ALARM1_HOUR) & 0x7f;
	DS3231_SetRegByte(DS3231_REG_ALARM1_HOUR, temp | (((alarmMode >> 2) & 0x01) << DS3231_BIT_AXMY));
	temp = DS3231_GetRegByte(DS3231_REG_ALARM1_DAY) & 0x7f;
	DS3231_SetRegByte(DS3231_REG_ALARM1_DAY, temp | (((alarmMode >> 3) & 0x01) << DS3231_BIT_AXMY) | (alarmMode & 0x80));
}
...
uint8_t DS3231_IsOscillatorStopped(){
	return (DS3231_GetRegByte(DS3231_REG_CNTR_STATUS) >> DS3231_BIT_OSF) & 0x01;
}
...
uint8_t DS3231_Is32kHzEnabled(){
	return (DS3231_GetRegByte(DS3231_REG_CNTR_STATUS) >> DS3231_BIT_EN32KHZ) & 0x01;
}
...
uint8_t DS3231_IsAlarm1Triggered(){
	return (DS3231_GetRegByte(DS3231_REG_CNTR_STATUS) >> DS3231_BIT_A1F) & 0x01;
}
...
uint8_t DS3231_IsAlarm2Triggered(){
	return (DS3231_GetRegByte(DS3231_REG_CNTR_STATUS) >> DS3231_BIT_A2F) & 0x01;
}
...
uint8_t DS3231_GetDayOfWeek(void) {
	return DS3231_DecodeBCD(DS3231_GetRegByte(DS3231_REG_DAY));
}
...
uint8_t DS3231_GetMonth(void) {
	return DS3231_DecodeBCD(DS3231_GetRegByte(DS3231_REG_MONTH_CENTERY) & 0x7f);
}
...
uint16_t DS3231_GetYear(void) {
	...
	uint16_t century = (DS3231_GetRegByte(DS3231_REG_MONTH_CENTERY) >> DS3231_BIT_CENTURY) * 100 + 2000;
...
uint8_t DS3231_GetHour(void) {
	return DS3231_DecodeBCD(DS3231_GetRegByte(DS3231_REG_HOURS));
}
...
uint8_t DS3231_GetMinute(void) {
	return DS3231_DecodeBCD(DS3231_GetRegByte(DS3231_REG_MINUTES));
}
...
uint8_t DS3231_GetSecond(void) {
	return DS3231_DecodeBCD(DS3231_GetRegByte(DS3231_REG_SECONDS));
}
...
void DS3231_SetDayOfWeek(uint8_t dayOfWeek) {
	DS3231_SetRegByte(DS3231_REG_DAY, DS3231_EncodeBCD(dayOfWeek));
}
...
void DS3231_SetMonth(uint8_t month) {
	uint8_t century = DS3231_GetRegByte(DS3231_REG_MONTH_CENTERY) & 0x80;
	DS3231_SetRegByte(DS3231_REG_MONTH_CENTERY, DS3231_EncodeBCD(month) | century);
}
...
void DS3231_SetYear(uint16_t year) {
	...
	uint8_t monthReg = (DS3231_GetRegByte(DS3231_REG_MONTH_CENTERY) & 0x7f) | (century << DS3231_BIT_CENTURY);
	DS3231_SetRegByte(DS3231_REG_MONTH_CENTERY, monthReg);
...
void DS3231_SetHour(uint8_t hour_24mode) {
	DS3231_SetRegByte(DS3231_REG_HOURS, DS3231_EncodeBCD(hour_24mode & 0x3f));
}
...
void DS3231_SetMinute(uint8_t minute) {
	DS3231_SetRegByte(DS3231_REG_MINUTES, DS3231_EncodeBCD(minute));
}
...
void DS3231_SetSecond(uint8_t second) {
	DS3231_SetRegByte(DS3231_REG_SECONDS, DS3231_EncodeBCD(second));
}
...
void DS3231_Enable32kHzOutput(DS3231_State enable){
	uint8_t status = DS3231_GetRegByte(DS3231_REG_CNTR_STATUS) & 0xfb;
	DS3231_SetRegByte(DS3231_REG_CNTR_STATUS, status | (enable << DS3231_BIT_EN32KHZ));
}
...
int8_t DS3231_GetTemperatureInteger(){
	return DS3231_GetRegByte(DS3231_REG_MSB_OF_TEMP);
}
...
uint8_t DS3231_GetTemperatureFraction(){
	return (DS3231_GetRegByte(DS3231_REG_LSB_OF_TEMP) >> 6) * 25;
}
...
HAL_StatusTypeDef  DS3231_ReadRegister(DS3231 *dev, uint8_t reg, uint8_t *data){
	...
	return HAL_I2C_Mem_Read(dev ->i2cHandle,DS3231_I2C_ADDR, reg,I2C_MEMADD_SIZE_8BIT, data, 1, HAL_MAX_DELAY);
}

HAL_StatusTypeDef  DS3231_ReadRegisters(DS3231 *dev, uint8_t reg, uint8_t *data, uint8_t length ){

	return HAL_I2C_Mem_Read(dev ->i2cHandle, DS3231_I2C_ADDR, reg,I2C_MEMADD_SIZE_8BIT, data,length, HAL_MAX_DELAY);
}

HAL_StatusTypeDef  DS3231_WridedRegister(DS3231 *dev, uint8_t reg, uint8_t *data){

	return HAL_I2C_Mem_Write(dev ->i2cHandle,DS3231_I2C_ADDR, reg,I2C_MEMADD_SIZE_8BIT, data, 1, HAL_MAX_DELAY);
}
